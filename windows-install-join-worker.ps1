...
function Join-Swarm ()
{
    ...
    docker image pull docker/ucp-agent-win:3.1.7
    docker image pull docker/ucp-dsinfo-win:3.1.7

    # Execute the local node configuration

    docker container run --rm docker/ucp-agent-win:3.1.7 windows-script | powershell -noprofile -noninteractive -command 'Invoke-Expression -Command $input'
